...
function GetWMIObjectsCount(Services:ISWbemServices; wmiClass:string):integer;

//: Devuelve el valor de una propiedad, a partir de la clase y su nombre.
procedure GetWMIValue(AIndex:integer;
...
function VariantStrValue(v:variant; vIsNull:Boolean):string;
begin
  if (vIsNull) then Result := STR_EMPTY
  else Result := v;
end;
...
//    // Install Date
//    Str := _ExtractValue('InstallDate');
//    if (Str <> '') then begin
//      // formatear
//      Str := Copy(Str, 7, 2) + '/' + Copy(Str, 5, 2) + '/' + Copy(Str, 1, 4);
//      // Añadir
//      sgProcesos.Cells[1, actual-1] := Str;
//    end;
//
//    // Vendor
//    Str := _ExtractValue('Vendor');
//    if (Str <> '') then begin
//      sgProcesos.Cells[2, actual-1] := Str;
//    end;
//
//    // Version
//    Str := _ExtractValue('Version');
//    if (Str <> '') then begin
//      sgProcesos.Cells[3, actual-1] := Str;
//    end;
...
begin
  ...
  if (AObject <> nil) then begin
    ...
    if Assigned(wmiInParams) then begin
      j := wmiInParams.Properties_.Get_Count;
    ...
    if Assigned(wmiOutParams) then begin
      j := wmiOutParams.Properties_.Get_Count;
