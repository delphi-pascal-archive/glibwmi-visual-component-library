...
procedure TFormMainProcessor.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := False;
...
begin
  ...
  if (ProcessorInfo1.ObjectsCount = 0) then begin
    Exit;
  end;
  ...
  for i := 1 to (ProcessorInfo1.ObjectsCount - 1) do begin
...
begin
  ...
  if (b) then begin
    rePrint.Clear;
    rePrint.Lines.Add(ProcessorInfo1.AllProperties[PageControl1.ActivePageIndex]);
    rePrint.Print('Información de ProcessorInfo (GLibWMI)');
  end;

end;

function TFormMainProcessor._CreateFrame(Index:integer; CreateTab:Boolean=True):TFrInfo;
var
  i:Integer;
  ts:TTabSheet;
  fr:TFrInfo;
